...
class DemandScraper(BaseScraper):
    ...
    async def scrape(self, request: ScrapeRequest) -> RawDemandGasFlowDataset:
        ...
        logger.info("Finished demand scraper with %s rows. %s", len(records), DEMO_DATA_NOTICE)
        return RawDemandGasFlowDataset(records=records, unit=UNIT, source_name="static_fake_demo")
